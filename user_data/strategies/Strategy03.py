...
class Strategy03(IStrategy):
    """
    WLD Pivot Point Mean Reversion
    --------------------------------
    ...
    def populate_indicators(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        # Replicate: h = max(highs[i-48:i]), l = min(lows[i-48:i]), c = closes[i-1]
        # rolling(48) looks back 48 completed bars; shift(1) gives previous close.
    ...
    def populate_exit_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        # Mean reversion achieved: price has returned to / crossed the pivot
        dataframe.loc[
